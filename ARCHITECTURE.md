...
**hadiscover** is a search engine for Home Assistant automations, built with a Python/FastAPI backend, Next.js frontend, and a release-hosted SQLite database generated by GitHub Actions.

## Stack

- **Backend**: Python 3.12+, FastAPI, SQLAlchemy, APScheduler for local/development indexing
- **Frontend**: Next.js 14, TypeScript, Tailwind CSS
- **Database**: SQLite with full-text search, built by GitHub Actions and published as a GitHub release asset
...
User Browser (Next.js)
       ↓ HTTP/JSON
FastAPI Backend (read/search API)
  ├─ Search Service
  └─ SQLite Database
       ↓
Release asset: hadiscover.db.gz
       ↑
...
5. **Publish**: Compress the SQLite database as `hadiscover.db.gz` and upload it to GitHub releases (`db-latest` plus dated snapshots)
6. **Continue**: Best-effort parsing; failures logged but don't stop indexing

### Backend Startup

1. If `DB_DOWNLOAD_URL` is set, download the gzip-compressed SQLite release asset
2. Validate it with SQLite `PRAGMA integrity_check`
3. Atomically replace the local database file
4. Initialize tables if needed and serve search/statistics requests
5. In production, set `DISABLE_SCHEDULER=true` so GitHub Actions remains the only indexing writer
...
### Release-Hosted SQLite Database

SQLite keeps the runtime simple and portable. GitHub Actions performs the write-heavy indexing job, then publishes the resulting database as a GitHub release asset. Production backends download that asset at startup and serve read/search traffic without needing a hosted database server or GitHub API token.
...
The `update-db` workflow runs on a schedule and by manual dispatch. It builds the database with `python -m app.cli index-now`, compresses it, and publishes rolling and dated GitHub release assets. APScheduler remains available for local development or self-hosted deployments that prefer in-process indexing.
