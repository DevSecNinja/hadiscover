...
# Set DB_DOWNLOAD_URL to the URL of a gzip-compressed SQLite database file.
# The database is only downloaded when no existing database file is present.
# Example:
#   DB_DOWNLOAD_URL=https://github.com/DevSecNinja/hadiscover/releases/download/db-latest/hadiscover.db.gz
if [ -n "${DB_DOWNLOAD_URL:-}" ] && [ ! -f "/app/data/hadiscover.db" ]; then
...
" "${DB_DOWNLOAD_URL}"; then
		...
		echo "Warning: Failed to download pre-built database. Starting with an empty database."
